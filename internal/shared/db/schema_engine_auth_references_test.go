...
func TestEngineSchemaRetiresWorkspaceAuthReferenceOwnership(t *testing.T) {
	...
	preflightSQL := strings.Join(appCredentialSourcePreflightQueries(), "\n")
	// Source identities move to sessions/connections before the incorrect global table is dropped.
	if !strings.Contains(cleanupSQL, "credential_source_service_id = reference.source_service_id") ||
		!strings.Contains(preflightSQL, "legacy workspace auth references contain an unsupported or incompatible credential family") ||
		!strings.Contains(cleanupSQL, "DROP TABLE IF EXISTS fused_workspace_auth_references") {
