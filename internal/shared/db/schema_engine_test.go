...
// TestEngineSchemaDefinesStableMCPFamilyRouting locks the shared App route pointer and its one-time convergence into the baseline.
func TestEngineSchemaDefinesStableMCPFamilyRouting(t *testing.T) {
	...
	assertSchemaContainsAll(t, schema, "stable MCP convergence missing %q", []string{
		"ADD COLUMN IF NOT EXISTS hosted_mcp boolean NOT NULL DEFAULT false",
		"DROP CONSTRAINT chk_fused_app_families_stable_mcp",
