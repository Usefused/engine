...
func TestWorkspaceConfigApplyHandler_UpsertsConnectConfigFromRuntimeConfig(t *testing.T) {
	svcID := uuid.New()
	bucketID := uuid.New()
	s := &workspaceTestStore{
		accountID:   uuid.New(),
	...
	planID := uuid.New()
	payload := workspaceConnectApplyPayload(svcID, "prod")
	configStore := workspaceApplyConfigStore(planID, payload)

	r := chi.NewRouter()
	r.Post("/workspace/config/apply", WorkspaceConfigApplyHandler(configStore, s, &mockRegistryClient{name: "GitHub"}, testMasterKey))
	req := httptest.NewRequest(http.MethodPost, "/workspace/config/apply", bytes.NewReader(workspaceConnectApplyRequest(planID)))
	req.Header.Set("X-API-Key", "fsk_test")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
	...
	if len(s.upsertedConnectConfigs) != 1 {
		t.Fatalf("expected one connect config upsert, got %#v", s.upsertedConnectConfigs)
	}
	assertConnectConfigStoredEncrypted(t, s.upsertedConnectConfigs[0], bucketID, svcID)
	if bytes.Contains(configStore.upserted.DesiredState, []byte("client-secret")) {
		t.Fatal("applied desired state must not store resolved connect client secret")
	}
}

func TestWorkspaceConfigApplyHandler_ReturnsBadRequestForUnknownConnectBucket(t *testing.T) {
	svcID := uuid.New()
	s := &workspaceTestStore{
		accountID:     uuid.New(),
		workspaceID:   uuid.New(),
	...
	planID := uuid.New()
	configStore := workspaceApplyConfigStore(planID, workspaceConnectApplyPayload(svcID, "missing"))
	...
	if len(s.upsertedConnectConfigs) != 0 {
		t.Fatalf("connect config should not be upserted for unknown bucket: %#v", s.upsertedConnectConfigs)
	}
	if s.gotVersion != "" {
		t.Fatalf("service version should not be activated before connect config validates, got %q", s.gotVersion)
	}
}

func TestWorkspaceConfigApplyHandler_ReusesSharedConnectBucketLookup(t *testing.T) {
	firstSvcID := uuid.New()
	secondSvcID := uuid.New()
	bucketID := uuid.New()
	s := &workspaceTestStore{
		accountID:   uuid.New(),
		workspaceID: uuid.New(),
	...
	planID := uuid.New()
	configStore := workspaceApplyConfigStore(planID, workspaceSharedConnectApplyPayload(firstSvcID, secondSvcID))

	r := chi.NewRouter()
	r.Post("/workspace/config/apply", WorkspaceConfigApplyHandler(configStore, s, &mockRegistryClient{name: "GitHub"}, testMasterKey))
	req := httptest.NewRequest(http.MethodPost, "/workspace/config/apply", bytes.NewReader(workspaceSharedConnectApplyRequest(planID)))
	req.Header.Set("X-API-Key", "fsk_test")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
	...
	if got := len(s.bucketLookupNames); got != 1 {
		t.Fatalf("expected one shared bucket lookup, got %d: %#v", got, s.bucketLookupNames)
	}
	if got := len(s.upsertedConnectConfigs); got != 2 {
		t.Fatalf("expected two connect config upserts, got %d: %#v", got, s.upsertedConnectConfigs)
	}
}
...
func workspaceConnectApplyPayload(svcID uuid.UUID, bucket string) json.RawMessage {
	payload := map[string]any{
		"kind":     "workspace",
		"services": workspaceTestServicePayload(svcID),
		"buckets": map[string]any{bucket: map[string]any{"service_config": map[string]any{"github": map[string]any{"connect": map[string]any{
			"auth_type": "oauth", "client_id": "$GITHUB_CLIENT_ID", "client_secret": "$GITHUB_CLIENT_SECRET", "redirect_uri": "https://engine.example.com/connect/callback",
		}}}}},
	}
...
func workspaceSharedConnectApplyPayload(firstSvcID, secondSvcID uuid.UUID) json.RawMessage {
	payload := map[string]any{
		"kind": "workspace",
		"services": map[string]any{
			"github": workspaceTestServiceEntry(firstSvcID),
			"linear": workspaceTestServiceEntry(secondSvcID),
		},
		"buckets": map[string]any{"prod": map[string]any{"service_config": map[string]any{
			"github": map[string]any{"connect": map[string]any{"auth_type": "oauth", "client_id": "$GITHUB_CLIENT_ID", "client_secret": "$GITHUB_CLIENT_SECRET", "redirect_uri": "https://engine.example.com/connect/callback"}},
			"linear": map[string]any{"connect": map[string]any{"auth_type": "oauth", "client_id": "$LINEAR_CLIENT_ID", "client_secret": "$LINEAR_CLIENT_SECRET", "redirect_uri": "https://engine.example.com/connect/callback"}},
		}}},
	}
	body, _ := json.Marshal(payload)
	return body
...
func workspaceConnectApplyRequest(planID uuid.UUID) []byte {
	return []byte(`{
...
func workspaceSharedConnectApplyRequest(planID uuid.UUID) []byte {
	return []byte(`{
		"plan_id":"` + planID.String() + `",
		"source_hash":"abc",
		"connect_materials": {
			"prod\u0000github": {"client_id": "github-client-id", "client_secret": "github-client-secret"},
			"prod\u0000linear": {"client_id": "linear-client-id", "client_secret": "linear-client-secret"}
		}
	}`)
}
...
func assertConnectConfigStoredEncrypted(t *testing.T, cfg store.ConnectConfig, bucketID, serviceID uuid.UUID) {
	t.Helper()
	assertConnectConfigIdentity(t, cfg, bucketID, serviceID)
	assertConnectConfigCiphertexts(t, cfg)
	assertConnectConfigDecrypts(t, cfg)
}

func assertConnectConfigIdentity(t *testing.T, cfg store.ConnectConfig, bucketID, serviceID uuid.UUID) {
	t.Helper()
	if cfg.BucketID != bucketID || cfg.ServiceID != serviceID || cfg.AuthType != "oauth" {
		t.Fatalf("unexpected connect config identity: %#v", cfg)
	}
}

func assertConnectConfigCiphertexts(t *testing.T, cfg store.ConnectConfig) {
	t.Helper()
	if cfg.EncryptedClientID == "client-id" || cfg.EncryptedClientSecret == "client-secret" {
		t.Fatalf("connect config credentials must be encrypted before storage: %#v", cfg)
	}
}

func assertConnectConfigDecrypts(t *testing.T, cfg store.ConnectConfig) {
	t.Helper()
	dek, err := store.UnwrapDEK(testMasterKey, cfg.EncryptedDEK)
	if err != nil {
		t.Fatalf("unwrap connect config DEK: %v", err)
	}
	assertConnectConfigFieldDecrypts(t, dek, cfg.EncryptedClientID, "client-id")
	assertConnectConfigFieldDecrypts(t, dek, cfg.EncryptedClientSecret, "client-secret")
}

func assertConnectConfigFieldDecrypts(t *testing.T, dek []byte, ciphertext, want string) {
	t.Helper()
	got, err := store.DecryptWithDEK(dek, ciphertext)
	if err != nil || got != want {
		t.Fatalf("decrypt connect config field = %q want %q err=%v", got, want, err)
	}
}

// ─── connect client_secret: ${bucket.<name>.secret.<key>} reference ───────

// TestValidateWorkspaceConnectClientSecret_AcceptsEnvAndAmbientBucketRef
// proves both supported forms pass grammar validation.
func TestValidateWorkspaceConnectClientSecret_AcceptsEnvAndAmbientBucketRef(t *testing.T) {
	cases := []string{"$GITHUB_CLIENT_SECRET", "${GITHUB_CLIENT_SECRET}", "${bucket.secret.GITHUB_SECRET}"}
	for _, value := range cases {
		if err := validateWorkspaceConnectClientSecret("github", value); err != nil {
			t.Errorf("validateWorkspaceConnectClientSecret(%q): unexpected error: %v", value, err)
		}
	}
}

// TestValidateWorkspaceConnectClientSecret_RejectsNamedBucketAndLiteral
// proves the named-bucket form is rejected (unlike kind: webhook) since a
// connect config already belongs to one specific bucket -- resolved before
// the secret ref is ever read -- so naming a different bucket here would
// only ever mean reading a secret out of the wrong bucket. An inline literal
// is rejected as before.
func TestValidateWorkspaceConnectClientSecret_RejectsNamedBucketAndLiteral(t *testing.T) {
	cases := []string{"", "literal-secret", "${bucket.prod.secret.GITHUB_SECRET}", "${bucket.prod.env.GITHUB_SECRET}"}
	for _, value := range cases {
		if err := validateWorkspaceConnectClientSecret("github", value); err == nil {
			t.Errorf("validateWorkspaceConnectClientSecret(%q): expected an error, got none", value)
		}
	}
}

// TestWorkspaceConfigApplyHandler_ResolvesConnectClientSecretFromBucketRef
// proves client_secret can be sourced from an already-stored bucket secret,
// resolved server-side at apply time, instead of requiring the raw value in
// connect_materials on every apply.
func TestWorkspaceConfigApplyHandler_ResolvesConnectClientSecretFromBucketRef(t *testing.T) {
	svcID := uuid.New()
	bucketID := uuid.New()
	wrappedDEK, dek, err := store.WrapDEK(testMasterKey)
	if err != nil {
		t.Fatalf("wrap DEK: %v", err)
	}
	encVal, err := store.EncryptWithDEK(dek, "resolved-from-bucket")
	if err != nil {
	...
	s := &workspaceTestStore{
		...
		secretsByKey: map[string]*store.WorkspaceSecret{
			"secret:GITHUB_SECRET": {EncryptedDEK: wrappedDEK, EncryptedValue: encVal},
		},
	}
	planID := uuid.New()
	payload := map[string]any{
		"kind":     "workspace",
		"services": workspaceTestServicePayload(svcID),
		"buckets": map[string]any{"prod": map[string]any{"service_config": map[string]any{"github": map[string]any{"connect": map[string]any{
			"auth_type": "oauth", "client_id": "client-id", "client_secret": "${bucket.secret.GITHUB_SECRET}", "redirect_uri": "https://engine.example.com/connect/callback",
		}}}}},
	}
	body, _ := json.Marshal(payload)
	configStore := workspaceApplyConfigStore(planID, body)

	r := chi.NewRouter()
	r.Post("/workspace/config/apply", WorkspaceConfigApplyHandler(configStore, s, &mockRegistryClient{name: "GitHub"}, testMasterKey))
	// No connect_materials for client_secret -- it must resolve from the store.
	req := httptest.NewRequest(http.MethodPost, "/workspace/config/apply", bytes.NewReader([]byte(`{"plan_id":"`+planID.String()+`","source_hash":"abc"}`)))
	req.Header.Set("X-API-Key", "fsk_test")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
	...
	cfg := s.upsertedConnectConfigs[0]
	got := decryptConnectSecretForTest(t, cfg)
	if got != "resolved-from-bucket" {
		t.Fatalf("expected client_secret resolved from the bucket secret, got %q", got)
	}
	if len(s.secretLookupKeys) != 1 || s.secretLookupKeys[0] != "secret:GITHUB_SECRET" {
		t.Fatalf("expected exactly one lookup for secret:GITHUB_SECRET, got %#v", s.secretLookupKeys)
	}
}

// TestWorkspaceConfigApplyHandler_ConnectClientSecretMissingBucketSecretFails
// proves a syntactically valid reference to a bucket with no matching secret
// fails apply with a clear error instead of silently storing an empty value.
func TestWorkspaceConfigApplyHandler_ConnectClientSecretMissingBucketSecretFails(t *testing.T) {
	svcID := uuid.New()
	s := &workspaceTestStore{
		accountID:     uuid.New(),
		workspaceID:   uuid.New(),
		bucketsByName: map[string]*store.Bucket{"prod": {ID: uuid.New(), Name: "prod"}},
	}
	planID := uuid.New()
	payload := map[string]any{
		"kind":     "workspace",
		"services": workspaceTestServicePayload(svcID),
		"buckets": map[string]any{"prod": map[string]any{"service_config": map[string]any{"github": map[string]any{"connect": map[string]any{
			"auth_type": "oauth", "client_id": "client-id", "client_secret": "${bucket.secret.NEVER_CONFIGURED}", "redirect_uri": "https://engine.example.com/connect/callback",
		}}}}},
	}
	body, _ := json.Marshal(payload)
	configStore := workspaceApplyConfigStore(planID, body)
	...
	if rr.Code == http.StatusOK {
		t.Fatalf("expected apply to fail for a missing bucket secret, got 200: %s", rr.Body.String())
	}
	if len(s.upsertedConnectConfigs) != 0 {
		t.Fatalf("expected no connect config written when the bucket secret is missing, got %#v", s.upsertedConnectConfigs)
	}
}

// TestWorkspaceConfigApplyHandler_RejectsNamedBucketConnectClientSecret
// proves a named-bucket reference in connect client_secret fails apply with
// a clear rejection, not a wrong-bucket lookup -- a connect config already
// belongs to one specific bucket, so only the ambient ${bucket.secret.<key>}
// form is accepted here (see ambientWorkspaceBucketSecretKey).
func TestWorkspaceConfigApplyHandler_RejectsNamedBucketConnectClientSecret(t *testing.T) {
	svcID := uuid.New()
	s := &workspaceTestStore{
		accountID:     uuid.New(),
		workspaceID:   uuid.New(),
	...
	payload := map[string]any{
		...
		"services": workspaceTestServicePayload(svcID),
		"buckets": map[string]any{"prod": map[string]any{"service_config": map[string]any{"github": map[string]any{"connect": map[string]any{
			"auth_type": "oauth", "client_id": "client-id", "client_secret": "${bucket.prod.secret.GITHUB_SECRET}", "redirect_uri": "https://engine.example.com/connect/callback",
		}}}}},
	}
	body, _ := json.Marshal(payload)
	configStore := workspaceApplyConfigStore(planID, body)
	...
	if rr.Code == http.StatusOK {
		t.Fatalf("expected apply to reject a named-bucket client_secret, got 200: %s", rr.Body.String())
	}
	if len(s.upsertedConnectConfigs) != 0 {
		t.Fatalf("expected no connect config written when client_secret names a bucket, got %#v", s.upsertedConnectConfigs)
	}
}

func decryptConnectSecretForTest(t *testing.T, cfg store.ConnectConfig) string {
	t.Helper()
	dek, err := store.UnwrapDEK(testMasterKey, cfg.EncryptedDEK)
	if err != nil {
		t.Fatalf("unwrap connect config DEK: %v", err)
	}
	got, err := store.DecryptWithDEK(dek, cfg.EncryptedClientSecret)
	if err != nil {
		t.Fatalf("decrypt connect config client_secret: %v", err)
	}
	return got
}
