...
// autoRegisterMockStore records GetWorkspaceIDForAccount/IsWorkspaceServiceEnabled/
...
func TestAutoRegisterImportedService_ActivatesWhenNotYetActivated(t *testing.T) {
	...
	autoRegisterImportedService(context.Background(), s, nil, accountID, "", body)

	if s.workspaceCalls != 1 {
		t.Fatalf("expected VerifyWorkspaceOwner called once, got %d", s.workspaceCalls)
	}
...
func TestAutoRegisterImportedService_MaterializesSnapshotBeforeActivation(t *testing.T) {
	...
	autoRegisterImportedService(context.Background(), s, fetcher, accountID, "user-api-key", body)
...
func TestAutoRegisterImportedService_MalformedJSONNoPanic(t *testing.T) {
	...
	autoRegisterImportedService(context.Background(), s, nil, s.accountID, "", []byte(`not json`))
...
func TestAutoRegisterImportedService_MissingServiceIDSkips(t *testing.T) {
	s := &autoRegisterMockStore{accountID: uuid.New()}
	autoRegisterImportedService(context.Background(), s, nil, s.accountID, "", []byte(`{"status":"applied"}`))
...
func TestAutoRegisterImportedService_StoreErrorsDoNotPanic(t *testing.T) {
	...
	t.Run("workspace resolution error", func(t *testing.T) {
		s := &autoRegisterMockStore{accountID: uuid.New(), workspaceErr: context.DeadlineExceeded}
		autoRegisterImportedService(context.Background(), s, nil, s.accountID, "", body)
		if s.activateCalls != 0 {
			t.Errorf("expected AddWorkspaceServiceVersion not reached after a workspace resolution error")
		}
	})
