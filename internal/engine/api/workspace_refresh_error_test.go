...
func TestWriteRefreshServiceContractErrorUsesSharedEnvelope(t *testing.T) {
	tests := []struct {
		...
		{name: "rejected contract", err: refreshHTTPError{status: http.StatusUnprocessableEntity, rejectedVersion: uuid.New(), rejectedMessage: "generation_contract_duplicate_record_id: The saved contract contains the same immutable Fused record ID more than once. Re-import the service from its original OpenAPI file or URL, then try again."}, status: http.StatusUnprocessableEntity, code: "runtime_contract_rejected", message: "Fused cannot use the saved API contract for this service version.", serverDetail: "generation_contract_duplicate_record_id: The saved contract contains the same immutable Fused record ID more than once. Re-import the service from its original OpenAPI file or URL, then try again."},
