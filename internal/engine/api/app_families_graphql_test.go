...
func TestAppFamiliesPreserveUIVersionQueries(t *testing.T) {
	for _, kind := range []store.AppKind{store.AppKindSDK, store.AppKindMCP} {
		...
		t.Run(string(kind), func(t *testing.T) {
			...
			fixture := &workspaceTestStore{accountID: account, mockScopes: map[uuid.UUID]*store.AppRuntime{
				oldID: {AccountID: account, AppFamilyID: family, AppID: oldID, StableAppID: oldID, Kind: kind, Name: "Support:Tools", Version: "1", CreatedAt: time.Now()},
				newID: {AccountID: account, AppFamilyID: family, AppID: newID, StableAppID: oldID, Kind: kind, Name: "Support:Tools", Version: "2", CreatedAt: time.Now()},
			}}
			s := &appFamilyGraphQLTestStore{artifactReferenceGraphQLTestStore: &artifactReferenceGraphQLTestStore{workspaceTestStore: fixture}, family: store.AppFamilyCatalogItem{AppFamilyID: family, Name: "Support:Tools", Kind: kind, VersionCount: 2, StableAppID: oldID, StableVersion: "1"}}
			h := mountMCPGraphQLTestHandler(t, s)
			data := doMCPGraphQLRequest(t, h, fmt.Sprintf(`query {
    appFamilies(kind:%q,limit:1,offset:0) { total items { app_family_id name version_count stable_version stable_version_id transport_urls { streamable_http sse } } }
    ...
    appVersions(app_family_id:%q) { app_id version }
			...
			// Grouping preserves the exact logical identity and complete live version count.
			if item["app_family_id"] != family.String() || item["version_count"] != float64(2) {
			...
			if kind == store.AppKindMCP {
				...
				// SDK discovery must not imply a default executable version.
...
func TestUnpromotedAppFamilyHasNoTransport(t *testing.T) {
	result := appFamilySummaryFields(nil, store.AppFamilyCatalogItem{AppFamilyID: uuid.New(), Name: "support", Kind: store.AppKindMCP, VersionCount: 1})
