...
func TestApplyArtifactConfigPlanConcurrentWinnerKeepsRuntimeAndToken(t *testing.T) {
		...
		t.Run(string(configType), func(t *testing.T) {
			...
			for index := range errorsByApply {
				...
				go func(index int) {
					...
					_, errorsByApply[index] = fixture.repository.ApplyArtifactConfigPlan(fixture.ctx, params)
				}(index)
			}
			close(start)
			wait.Wait()
...
func TestApplyArtifactConfigPlanUsesAuthorizationBeforeTeamLockOrder(t *testing.T) {
	...
	go func() {
		_, applyErr := fixture.repository.ApplyArtifactConfigPlan(fixture.ctx, params)
		done <- applyErr
	}()
...
func TestApplyArtifactConfigPlanRejectsSDKWithoutPlanLease(t *testing.T) {
	fixture := newConcurrentArtifactApplyFixture(t, ConfigTypeSDK)
	fixture.params.Plan.ApplyLeaseID = uuid.Nil
	fixture.params.TokenHash = "missing-lease-" + uuid.NewString()

	_, err := fixture.repository.ApplyArtifactConfigPlan(fixture.ctx, fixture.params)
...
func TestApplyArtifactConfigPlanRejectsSameNameBucketReplacementAtomically(t *testing.T) {
	fixture := newConcurrentArtifactApplyFixture(t, ConfigTypeSDK)
	...
	if !errors.Is(err, ErrSDKBucketImmutable) {
		t.Fatalf("ApplyArtifactConfigPlan(stale bucket) = %v, want ErrSDKBucketImmutable", err)
	}
...
func assertRejectedArtifactApplyState(t *testing.T, fixture concurrentArtifactApplyFixture, replacementID uuid.UUID) {
	...
	checks := []struct {
		...
		{`SELECT COUNT(*) FROM fused_artifact_scopes WHERE artifact_id = $1`, fixture.params.Scope.ArtifactID, &scopes},
		{`SELECT COUNT(*) FROM fused_artifact_tokens WHERE artifact_id = $1`, fixture.params.Scope.ArtifactID, &tokens},
		{`SELECT COUNT(*) FROM fused_config_states WHERE latest_resource_id = $1`, fixture.params.Scope.ArtifactID, &states},
...
type concurrentArtifactApplyFixture struct {
	...
	pool       *pgxpool.Pool
	params     ApplyArtifactConfigPlanParams
}
	...
	return concurrentArtifactApplyFixture{
		...
		params: ApplyArtifactConfigPlanParams{
			Plan: ApplyConfigPlanParams{
				State:  UpsertConfigStateParams{ConfigKey: configKey, ConfigType: configType, SourceHash: "source", DesiredState: []byte("{}"), ManagedResources: []byte("{}"), LatestResourceID: &artifactID, UpdatedBy: accountID},
				PlanID: plan.ID, BaseGeneration: plan.BaseGeneration, ExpectedRevision: plan.Revision, ApplyLeaseID: lease.ID,
			},
			Scope:                ArtifactScope{AccountID: accountID, ArtifactID: artifactID, OwnerTeamID: ownerTeamID, BucketID: bucketID, Selections: []byte("[]"), ScopeSchemaVersion: 2, Kind: string(configType), ConfigKey: configKey},
			AuthorizedBucketName: bucketName,
			TokenName:            "default", Activate: true,
		},
	}
}
