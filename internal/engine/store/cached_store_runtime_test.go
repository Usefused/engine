...
func TestCachedStoreAppFamilyServiceBucketMutationInvalidates(t *testing.T) {
	...
	if err := cached.SetAppFamilyServiceBucket(context.Background(), familyID, serviceID, overrideBucket); err != nil {
		t.Fatalf("SetAppFamilyServiceBucket: %v", err)
	}
	// A committed override must force the next read to reload rather than
	// serve the pre-override cached projection.
