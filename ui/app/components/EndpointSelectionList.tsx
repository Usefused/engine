...
function matchesSearchTerm(ep: IntegrationObject, term: string): boolean {
  return Boolean(
...
function responseCodeColors(code: string) {
  if (code.startsWith('2')) return { badge: 'bg-emerald-50 text-emerald-600 border border-emerald-100', dot: 'bg-emerald-400' };
  if (code.startsWith('4') || code.startsWith('5')) return { badge: 'bg-rose-50 text-rose-600 border border-rose-100', dot: 'bg-rose-400' };
  return { badge: 'bg-slate-50 text-slate-600 border border-slate-200', dot: 'bg-slate-400' };
}

function ResponseCodeBadges({ responses }: { responses?: Record<string, unknown> }) {
  if (!responses || Object.keys(responses).length === 0) return null;
  return (
    <div className="mt-1.5 flex flex-wrap gap-1">
      {Object.keys(responses).map(code => {
        const { badge, dot } = responseCodeColors(code);
        return (
          <span key={code} title={`Response schema captured for HTTP ${code}`} className={`text-[9px] font-bold px-1.5 py-0.5 rounded flex items-center gap-1 ${badge}`}>
...
interface EndpointSelectionRowProps {
  ep: IntegrationObject;
  id: string;
  isSelected: boolean;
  disabled: boolean;
...
function EndpointSelectionRow({ ep, id, isSelected, disabled, onToggle }: EndpointSelectionRowProps) {
  return (
    <label
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1 font-sans">
          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded shadow-sm ${ep.method ? (METHOD_BADGE_COLORS[ep.method] ?? "bg-slate-100 text-slate-700 border border-slate-200") : "bg-slate-100 text-slate-700 border border-slate-200"}`}>
            {ep.method}
          </span>
        ...
        <ResponseCodeBadges responses={ep.responses} />
      </div>
    </label>
  );
}
    ...
    <div className="bg-white">
      ...
      {!isCollapsed && (
        <div className="divide-y divide-slate-100">
          ...
          {!isLoading && !isStub && hasMore && onLoadMore && (
            <ObserverTarget disabled={false} onIntersect={onLoadMore} />
          )}
        </div>
      )}
    </div>
...
export default function EndpointSelectionList({
  ...
  const filtered = endpoints.filter(ep => {
    ...
    return matchesSearchTerm(ep, searchTerm.toLowerCase());
  });
          ...
          allResourceNames.map((resource) => {
            ...
            return (
              <ResourceGroupSection
                ...
                isStub={isStub}
                hasMore={Boolean(hasMoreResources[resource])}
                onHeaderClick={() => handleResourceClick(resource)}
                onLoadMore={onLoadMoreResource ? () => onLoadMoreResource(resource) : undefined}
